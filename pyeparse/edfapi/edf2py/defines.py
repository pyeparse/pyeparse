...
@author: Sol
"""
def createConstants(var_val_dict):
    ...
    constants = dict([(k, v) for k, v in dict(var_val_dict).iteritems() if (
        not k.startswith("_") and not callable(v))])
    constants.update([(v,k) for k, v in dict(constants).iteritems()])
    return constants

#************* EVENT TYPE CODES ************/
def createEventConstants():
    SAMPLE_TYPE = 200
    ...
    return createConstants(locals())
    
event_constants=createEventConstants()
...
def createEyeTypeConstants():
    ...
    return createConstants(locals())
eye_constants=createEyeTypeConstants()
...
edf_constants = dict([(k, v) for k, v in dict(locals()).iteritems() if (
